...
func ExampleRepository() {
	...
	defer l.Close()
	go http.Serve(l, nil)
	stream, err := eventsource.Subscribe("http://127.0.0.1:8080/articles", "", 0*time.Second)
	if err != nil {
		return
	}
	go buildRepo(srv)
	...
	stream, err = eventsource.Subscribe("http://127.0.0.1:8080/articles", "1", 0*time.Second)
	if err != nil {
		fmt.Println(err)
		return
	}
